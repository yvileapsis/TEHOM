...
[<AutoOpen>]
module TehomExtensions =
    type Game with
        member this.GetModel world = this.GetModelGeneric<Tehom> world
        member this.SetModel value world = this.SetModelGeneric<Tehom> value world
        member this.Model = this.ModelGeneric<Tehom> ()
...
type TehomDispatcher () =
    inherit GameDispatcher<Tehom, TehomMessage, TehomCommand> (Splash)
    ...
    override this.Message (model, message, _, world) =
        ...
        | ShowGameplay -> just (Gameplay Gameplay.empty)
        | Update ->
            match model with
            | Gameplay gameplay ->
