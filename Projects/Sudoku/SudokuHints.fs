...
module SudokuHints =
    ...
    let private tryMakeEliminationHint (technique : HintTechnique) (region : HintRegion) (eliminations : (Vector2i * Set<int>) list) (state : SudokuBoardState) =
    ...
    let private fullHousesInUnit (technique : HintTechnique) (region : HintRegion) (positions : Vector2i list) (state : SudokuBoardState) =
        let emptyPositions = positions |> List.filter (fun position -> state.Puzzle[position.Y, position.X] = 0)
        match emptyPositions with
        | [position] ->
            let existing = set [for position in positions do if state.Puzzle[position.Y, position.X] <> 0 then yield state.Puzzle[position.Y, position.X]]
    ...
    let private fullHouseRows (state : SudokuBoardState) =
        [0 .. 8]
        |> List.collect (fun row ->
            fullHousesInUnit FullHouseRow (HintRow row) (SudokuGrid.rowPositions row) state)

    let private fullHouseColumns (state : SudokuBoardState) =
        [0 .. 8]
        |> List.collect (fun column ->
            fullHousesInUnit FullHouseColumn (HintColumn column) (SudokuGrid.columnPositions column) state)

    let private fullHouseBlocks (state : SudokuBoardState) =
        [for y in 0 .. 2 do for x in 0 .. 2 -> v2i x y]
        |> List.collect (fun block ->
            fullHousesInUnit FullHouseBlock (HintBlock block) (SudokuGrid.blockPositions block) state)

    let private nakedSingles (state : SudokuBoardState) =
        SudokuGrid.allPositions
    ...
    let private hiddenSinglesInRows (state : SudokuBoardState) =
        [for row in 0 .. 8 do
            for number in 1 .. 9 do
                let positions =
                    [for x in 0 .. 8 do
                        let position = v2i x row
                        if Set.contains number (SudokuGrid.workingCandidates state position) then yield position]
    ...
    let private hiddenSinglesInColumns (state : SudokuBoardState) =
    ...
    let private hiddenSinglesInBlocks (state : SudokuBoardState) =
    ...
    let private nakedSubsetsInUnit (technique : HintTechnique) (region : HintRegion) (size : int) (positions : Vector2i list) (state : SudokuBoardState) =
        ...
        SudokuGrid.combinations size candidatesByPosition
            ...
            if Set.count subsetCandidates = size then
                ...
                let eliminations =
                    [for position in positions do
                        if not (Set.contains position subsetPositions) && state.Puzzle[position.Y, position.X] = 0 then
                            yield (position, subsetCandidates)]
                tryMakeEliminationHint technique region eliminations state
            else None)

    let private nakedPairsInRows (state : SudokuBoardState) =
        [0 .. 8]
        |> List.collect (fun row ->
            nakedSubsetsInUnit NakedPairRow (HintRow row) 2 (SudokuGrid.rowPositions row) state)

    let private nakedPairsInColumns (state : SudokuBoardState) =
        [0 .. 8]
        |> List.collect (fun column ->
            nakedSubsetsInUnit NakedPairColumn (HintColumn column) 2 (SudokuGrid.columnPositions column) state)

    let private nakedPairsInBlocks (state : SudokuBoardState) =
        [for y in 0 .. 2 do for x in 0 .. 2 -> v2i x y]
        |> List.collect (fun block ->
    ...
    let private nakedTriplesInRows (state : SudokuBoardState) =
        [0 .. 8]
        |> List.collect (fun row ->
            nakedSubsetsInUnit NakedTripleRow (HintRow row) 3 (SudokuGrid.rowPositions row) state)

    let private nakedTriplesInColumns (state : SudokuBoardState) =
        [0 .. 8]
        |> List.collect (fun column ->
            nakedSubsetsInUnit NakedTripleColumn (HintColumn column) 3 (SudokuGrid.columnPositions column) state)

    let private nakedTriplesInBlocks (state : SudokuBoardState) =
        [for y in 0 .. 2 do for x in 0 .. 2 -> v2i x y]
        |> List.collect (fun block ->
    ...
    let private pointingRowsOrColumns (state : SudokuBoardState) =
        [for blockY in 0 .. 2 do
            for blockX in 0 .. 2 do
                let block = v2i blockX blockY
                let blockPositions = SudokuGrid.blockPositions block
                for number in 1 .. 9 do
                    let positions =
                        blockPositions
                        |> List.filter (fun position -> Set.contains number (SudokuGrid.workingCandidates state position))
                    if List.length positions >= 2 then
                        ...
                        if Set.count rows = 1 then
                            ...
                            let eliminations =
                                [for position in SudokuGrid.rowPositions row do
                                    if SudokuGrid.blockOfPosition position <> block && Set.contains number (SudokuGrid.workingCandidates state position) then
                        ...
                        if Set.count columns = 1 then
                            ...
                            let eliminations =
                                [for position in SudokuGrid.columnPositions column do
                                    if SudokuGrid.blockOfPosition position <> block && Set.contains number (SudokuGrid.workingCandidates state position) then
                                        yield (position, set [number])]
    ...
    let private claimingRowsOrColumns (state : SudokuBoardState) =
        let rowClaims =
            [for row in 0 .. 8 do
                let rowPositions = SudokuGrid.rowPositions row
                for number in 1 .. 9 do
                    let positions =
                        rowPositions
                        |> List.filter (fun position -> Set.contains number (SudokuGrid.workingCandidates state position))
                    if List.length positions >= 2 then
                        let blocks = positions |> List.map SudokuGrid.blockOfPosition |> Set.ofList
                        if Set.count blocks = 1 then
                            let block = Set.minElement blocks
                            let eliminations =
                                [for position in SudokuGrid.blockPositions block do
                                    if position.Y <> row && Set.contains number (SudokuGrid.workingCandidates state position) then
                                        yield (position, set [number])]
                            yield (ClaimingRow, HintCells (Set.ofList positions), eliminations)]
        let columnClaims =
            [for column in 0 .. 8 do
                let columnPositions = SudokuGrid.columnPositions column
                for number in 1 .. 9 do
                    let positions =
                        columnPositions
                        |> List.filter (fun position -> Set.contains number (SudokuGrid.workingCandidates state position))
                    if List.length positions >= 2 then
                        ...
                        if Set.count blocks = 1 then
                            ...
                            let eliminations =
                                [for position in SudokuGrid.blockPositions block do
                                    if position.X <> column && Set.contains number (SudokuGrid.workingCandidates state position) then
                                        yield (position, set [number])]
    ...
    let private fishRows (size : int) (technique : HintTechnique) (state : SudokuBoardState) =
        [for number in 1 .. 9 do
            let rows =
                [for row in 0 .. 8 do
                    let columns =
                        [for position in SudokuGrid.rowPositions row do
                            if Set.contains number (SudokuGrid.workingCandidates state position) then yield position.X]
                    if List.length columns >= 2 && List.length columns <= size then yield (row, Set.ofList columns)]
            for rowSet in SudokuGrid.combinations size rows do
                        ...
                        [for column in columns do
                            for row in 0 .. 8 do
                                if not (Set.contains row selectedRows) then
                                    ...
                                    if Set.contains number (SudokuGrid.workingCandidates state position) then
                                        yield (position, set [number])]
                    let regionPositions =
    ...
    let private fishColumns (size : int) (technique : HintTechnique) (state : SudokuBoardState) =
        [for number in 1 .. 9 do
            let columns =
                [for column in 0 .. 8 do
                    let rows =
                        [for position in SudokuGrid.columnPositions column do
                            if Set.contains number (SudokuGrid.workingCandidates state position) then yield position.Y]
                    if List.length rows >= 2 && List.length rows <= size then yield (column, Set.ofList rows)]
            for columnSet in SudokuGrid.combinations size columns do
    ...
    let findAll (allowFish : bool) (state : SudokuBoardState) =
        hintGroups allowFish
        |> List.collect (fun find -> find state)

    let findFirst (allowFish : bool) (state : SudokuBoardState) =
        hintGroups allowFish
