...
module SlugDemoEmoji =
    ...
    let private emojiFontPaths =
        ...
        |> Array.collect (fun fileName ->
            [| Path.Combine ("Projects", "Slug Showcase", "Assets", "Default", fileName)
                ...
                try
                    ...
                    | Some glyphId ->
                        try
                            ...
                            let bounds = SlugShowcaseContours.getCompositeLayerBounds composite
                            Some
                                { CodePoint = codePoint
                                  Composite = composite
                                  Bounds = bounds
    ...
    let draw (world : World) =
        ...
        for index in 0 .. grid.Length - 1 do
            ...
            | Some glyph ->
                let column = index % 3
                let row = index / 3
                SlugShowcaseContours.placeCompositeInBounds
