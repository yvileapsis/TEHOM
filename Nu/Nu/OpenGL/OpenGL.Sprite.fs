...
module Text =
    ...
    type [<ReferenceEquality>] TextBatchEnv =
        private
            { mutable test : int }

    /// Create a sprite shader.
    let CreateTextShader (shaderFilePath : string) =
    ...
    /// Create a sprite quad for rendering to a shader matching the one created with Hl.CreateSpriteShader.
    let CreateTextQuad (string : (Char * Vector2) array) =


        // build vertex data
        let vertexData =
            string
            |> Array.map (fun (char, pos) ->
                [| float32 (char); pos.X |]
            )
            |> Array.concat
        ...
        let vertexSize = sizeof<single> * 2
        let vertexDataSize = vertexData.Length * sizeof<single>
        let vertexDataPtr = GCHandle.Alloc (vertexData, GCHandleType.Pinned)

        // Attribute location 2: vec2 Position
        Gl.EnableVertexAttribArray 0u
        Gl.VertexAttribPointer (0u, 2, VertexAttribPointerType.Float, false, vertexSize, nativeint 0)
        Gl.VertexAttribDivisor (0u, 1u)
