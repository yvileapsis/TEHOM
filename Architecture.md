...
The public game-facing layer is mostly under `Nu/Nu/World`, backed by lower-level service modules.
...
Game code usually exposes custom state as lenses through extension modules:
...
Facets dynamically augment entity behavior in a composable way. They can participate in registration, physics registration, update, render, ray casting, inferred attributes, and editor behavior.
...
`NuPlugin` is defined in [WorldTypes.fs](/Users/yvileapsis/Documents/GitHub/Nu/Nu/Nu/World/WorldTypes.fs). A game plugin lets the engine discover game-specific dispatchers and configure runtime behavior.
...
## Applied MMCC Game Architecture Playbook

This section is a self-contained recipe for building a larger Nu game in the MMCC style. It is derived from a complete jRPG built on Nu, but it does not require access to that project's source. Treat the names below as roles to recreate in a new project, not as dependencies.

The core idea is simple:

1. The top-level game model is a route union.
2. `Game.DesiredScreen` is derived from that route union.
3. Every major screen has its own model, messages, commands, and content.
4. Screens publish typed events when they need route changes.
5. The top-level game dispatcher performs cross-screen data transfer.
6. Screen lifecycle events, especially deselecting, are used to time safe handoffs.

### Recommended Root Shape For A Serious Game
...
The names are flexible, but the ownership boundaries should stay clear:

- `Program.fs`: startup only.
- `Assets.fs`: typed asset package names and asset tags.
- `Events.fs`: typed event addresses.
- `Simulants.fs`: canonical handles to screens, groups, entities, and important controls.
- `Data.fs`: centralized, typed load of CSV / symbolic / external content.
- `Game.fs`: top-level route model and game dispatcher.
- `GamePlugin.fs`: dispatcher registration and initial package list.
- Screen modules: screen-local models, messages, commands, and content.
- Entity modules: reusable actors, props, widgets, effects, and custom renderers.
- `.nugroup` files: editor-authored static menus, cutscenes, and layouts.
- `Overlayer.nuol`: shared UI styling defaults.

### Route Union

For a game with title, save picking, intro, exploration, and combat, the top-level route model should be an explicit union:
...
type GameRoute =
    ...
    | Credits
    | PickSaveSlot
    | Intro of saveSlot : SaveSlot
    | Field
    | Battle
```

This route model should not contain the entire field or battle state. It only answers, "Which major mode should the application desire right now?"

The top-level game dispatcher maps route to desired screen:

```fsharp
Game.DesiredScreen :=
    match route with
    | Splash -> Simulants.Splash
    | Title -> Simulants.Title
    | Credits -> Simulants.Credits
    | PickSaveSlot -> Simulants.Pick
    | Intro _ -> Simulants.Intro
    | Field -> Simulants.Field
    | Battle -> Simulants.Battle
```

This is the central screen-management pattern. Screens are declared once; selection is controlled by a typed route model.

### Route Messages And Commands

Use top-level messages for route decisions and top-level commands for cross-screen side effects.
...
type GameMessage =
    ...
    | ShowCredits
    | ShowPickSaveSlot
    | StartNewGame of SaveSlot
    | TryLoadGame of SaveSlot
    | ShowInitialField of SaveSlot
    | FieldIsStartingBattle
    | FieldBattleHandoff of BattleData * PrizePool
    | BattleIsConcluding of BattleOutcome
    | BattleConclusionHandoff
    | ExitGame
```

```fsharp
type GameCommand =
    | InitializeField of SaveSlot
    | LoadField of SaveSlot
    | TransferFieldToBattle of BattleData * PrizePool
    | TransferBattleToField of PrizePool
    | ClearBattle
...
- Messages change route state.
- Commands read / write screen models, publish events, update UI controls, save/load files, synchronize audio, and exit the process.

For maintainability, do not let screens directly mutate each other's models. Let screens publish facts, and let the top-level game command perform integration.

### Canonical Handles

Create one module that names every important screen and child object:
...
module Simulants =
    ...
    let FieldProps = Field / Group "Props"
    let FieldAvatar = Field / Entity "Avatar"

    let BattleCharacters = Battle / Group "Characters"
    let BattleEffects = Battle / Group "Effects"
    let BattleGui = Battle / Group "Gui"

    let TitleNewGame = Title / Entity "New"
    let TitleLoadGame = Title / Entity "Load"
    let TitleCredits = Title / Entity "Credits"
    let TitleExit = Title / Entity "Exit"
```

Rules:

- Gameplay code imports handles instead of reconstructing string addresses.
- If a name changes, update the handle module first.
- Cross-module references should use handles, not ad hoc `Screen "..."` calls.

### Canonical Events

Create one module for game-level and reusable UI events:

```fsharp
module Events =

    let QuitCreditsEvent = stoa<unit> "QuitCredits/Event"
    let QuitFieldEvent = stoa<unit> "QuitField/Event"

    let FieldIsStartingBattleEvent =
        stoa<unit> "Field/StartingBattle/Event"

    let FieldBattleHandoffEvent =
        stoa<BattleData * PrizePool> "Field/BattleHandoff/Event"

    let BattleIsConcludingEvent =
        stoa<BattleOutcome> "Battle/Concluding/Event"

    let BattleConclusionHandoffEvent =
        stoa<unit> "Battle/ConclusionHandoff/Event"

    let CancelEvent = stoa<unit> "Cancel/Event"
    let ItemSelectEvent = stoa<ItemId> "ItemSelect/Event"
    let TargetSelectEvent = stoa<CharacterId> "TargetSelect/Event"
```

Events should say what happened, not secretly command another screen. For example, `FieldIsStartingBattleEvent` is better than `ShowBattleScreenEvent`; the top-level game decides which route follows from that fact.

### Declaring Screens

Declare all major screens in the top-level game content:

```fsharp
override this.Content (route, game) =
    [Content.screenFromFile "Title" "Title.nugroup"
        [Screen.DispatcherName == "TitleDispatcher"]

     Content.screenFromFile "Pick" "Pick.nugroup" []

     Content.screenFromFile "Intro" "Intro.nugroup" []

     Content.screen "Field"
        [Screen.DispatcherName == "FieldDispatcher"]
        [Content.group "Props" [] []
         Content.entity "Avatar"
            [Entity.DispatcherName == "AvatarDispatcher"] []]

     Content.screen "Battle"
        [Screen.DispatcherName == "BattleDispatcher"]
        [Content.group "Characters" [] []
         Content.group "Effects" [] []
         Content.group "Gui" [] []]]
```

Use file-backed screens for static UI and presentation. Use custom dispatcher screens for gameplay modes whose content is generated from the model.

### Screen Dispatcher Shape

A substantial screen should have model, message, command, definitions, fallback model, and content:
...
type FieldDispatcher () =
    ...
    override this.GetFallbackModel (screen, world) =
        ...
        then FieldModel.initial world.UpdateTime
        else FieldModel.empty

    override this.Definitions (model, screen, world) =
        [screen / Screen.UpdateEvent => Update
         screen / Screen.PostUpdateEvent => PostUpdate
         screen / Screen.TimeUpdateEvent => UpdateTime
         screen / Screen.SelectEvent => Select
         screen / Screen.DeselectingEvent => Deselect
         screen / Screen.TransitionEvent => SetScreenTransitioning true
         screen / Screen.TransitionFinishedEvent => SetScreenTransitioning false
         Simulants.FieldAvatar / BodyTransformEvent =|> UpdateAvatarTransform
         Simulants.FieldAvatar / BodyPenetrationEvent =|> BodyPenetration
         Simulants.FieldAvatar / BodySeparationEvent =|> BodySeparation]

    override this.Message (model, message, screen, world) =
        match message with
        | Update -> FieldModel.update model world
        | Select -> FieldModel.select model
        | SetScreenTransitioning transitioning ->
            just { model with ScreenTransitioning = transitioning }
        | Deselect ->
            match model.State with
            | StartingBattle handoff -> withCommand (PublishBattleHandoff handoff) model
            | Quitting -> withCommand PublishQuit model
            | Playing -> just model

    override this.Command (model, command, screen, world) =
        match command with
        | PublishBattleHandoff handoff ->
            World.publishPlus handoff Events.FieldBattleHandoffEvent screen world
        | PublishQuit ->
            World.publishPlus () Events.QuitFieldEvent screen world
...
The exact operators and helper names depend on the Nu API in use, but the architecture is stable:

- `Definitions` subscribes to Nu lifecycle and child events.
- `Message` updates the screen model.
- `Command` performs world side effects.
- `Content` describes current entities from current state.
- `GetFallbackModel` protects screens whose models must survive while not selected.

### Two-Phase Field To Battle Handoff

For any game with an exploration screen and a combat screen, do not transfer the battle state in a single click or update. Use a two-phase handoff.

Phase 1: Field decides that a battle should start.

```text
Field update detects encounter.
Field model becomes StartingBattle (battleData, prizePool).
Field publishes FieldIsStartingBattleEvent.
Top-level game receives event.
Top-level route changes to Battle.
Nu begins selecting Battle and deselecting Field.
```

Phase 2: Field reaches the deselecting lifecycle event.

```text
Field Screen.DeselectingEvent fires.
Field sees State = StartingBattle handoff.
Field publishes FieldBattleHandoffEvent handoff.
Top-level game receives handoff event.
Top-level command TransferFieldToBattle runs.
```

The transfer command should:

1. Read the current Field model.
2. Capture any state that must resume later, such as music time or field transition state.
3. Construct the Battle model from field context and battle data.
4. Park or pause the Field model.
5. Write the Battle model to the Battle screen.
6. Write the parked Field model back to the Field screen.

This avoids a timing bug where the target screen receives data before the source screen has reached a safe lifecycle point.

### Two-Phase Battle To Field Handoff

Battle conclusion should also be lifecycle-aware.

Battle should represent conclusion explicitly:
...
type BattleState =
    ...
    | BattleRunning
    | BattleResult
    | BattleConcluding of startTime : int64 * outcome : BattleOutcome
    | BattleConclude of outcome : BattleOutcome

type BattleOutcome =
    | Win of PrizePool
    | Lose
    | Retry of BattleData * PrizePool
```

When Battle reaches the concluding state, it publishes a route fact:

```text
Battle publishes BattleIsConcludingEvent outcome.
Top-level game chooses desired route:
  Win -> Field
  Lose -> Title
  Retry -> Battle
```

Then, from Battle's `Screen.DeselectingEvent`, publish the final handoff event:

```text
Battle publishes BattleConclusionHandoffEvent.
Top-level command reads Battle and Field models.
On Win: apply prizes, consequences, experience, flags, and field state.
On Lose: clear battle and route to title or game-over.
On Retry: rebuild battle from stored retry data.
```

The source screen owns the outcome; the top-level game owns the cross-screen mutation.

### Local Transitions Inside A Screen

Not every transition should be a Nu screen change. Map-to-map travel inside an exploration mode is usually better as state inside the Field screen.
...
type FieldModel =
    { FieldId : FieldId
      Avatar : Avatar
      TransitionOpt : FieldTransition option
      ScreenTransitioning : bool }
```

Update pattern:

```text
If there is no local transition:
  accept movement, interaction, menu, and travel input.

If a local transition is active:
  fade out.
  at midpoint, load / generate destination field.
  swap field id.
  warp avatar to destination portal.
  update music.
  fade in.
  clear transition at completion.
```

Use top-level Nu screens for application modes. Use model-level transitions for travel within one mode.

### Gating Input During Transitions

Gameplay screens should usually track two different transition states:

- `ScreenTransitioning`: Nu is selecting or deselecting this screen.
- local transition state: the screen itself is fading, warping, loading, or changing maps.

Input should be gated when either is active. This prevents movement, menus, interactions, and battle triggers from firing while the presentation and model are between states.

### Save Model

Use a save-facing model instead of serializing the entire runtime screen model.
...
type FieldSave =
    { SaveSlot : SaveSlot
      FieldId : FieldId
      SeedState : SeedState
      AvatarState : AvatarSave
      Team : TeamSave
      Inventory : Inventory
      Flags : Set<GameFlag>
      Automaps : Map<FieldId, Automap> }
```

Runtime-only values should usually not be saved:

- current physics contacts
- temporary effects
- currently open dialog
- currently open shop confirmation
- live prop instances
- generated render descriptors
- active input state
- screen transition flags
- battle-local temporary UI

Rebuild those from data tables, seed state, and durable gameplay flags.

### Data Loading

Large games should centralize external content into one typed value:

```fsharp
type GameData =
    { Items : Map<ItemId, ItemData>
      Characters : Map<CharacterId, CharacterData>
      Battles : Map<BattleId, BattleData>
      Encounters : Map<EncounterId, EncounterData>
...
Runtime models should store typed ids and playthrough state. They should not duplicate entire rows from data files unless the row is deliberately snapshotted for balance or save compatibility.

### Entity Dispatchers

Use custom entity dispatchers for model-bearing actors and reusable widgets:

- Avatar / player dispatcher: physics body, movement render, animation render, transform events.
- Prop dispatcher: static or sensor physics body, interaction state, data-driven rendering.
- Character dispatcher: battle pose, animation, health indicators, targeting state.
- Menu dispatcher: selection logic, cancel / confirm events, visual layout.
- Reticle dispatcher: target selection events.
- Effect dispatcher: short-lived visual effect, often self-destructing.

The owning screen should remain the source of gameplay truth. Entity dispatchers encapsulate local presentation, local input, physics shape, and reusable behavior.

### Reusable Widget Flow

Reusable UI entities should publish typed events upward:

```text
Ring menu entity:
  player selects item
  publish ItemSelectEvent itemId

Battle screen:
  maps ItemSelectEvent to BattleMessage.SelectItem itemId

Battle message update:
  validates selection against battle state
  updates Battle model
```

Widgets should not directly mutate the parent screen. This keeps them reusable across screens and games.

### Asset And UI Styling

Use the asset graph to separate packages by purpose:

- `Default`: engine / project defaults.
- `Gui`: fonts, buttons, panels, cursors, shared UI sounds.
- `Field`: field tiles, props, avatars, exploration audio.
- `Battle`: battle sprites, effects, battle audio.
- `Data`: CSV / symbolic data.
- Environment-specific packages: castle, town, dungeon, world map, and so on.

Use an overlayer file for shared UI defaults such as fonts, button images, text colors, click sounds, toggle styling, and slice margins. This keeps menus visually consistent without repeating styling on every control.

### Audio Across Screens

Let screens own their normal background music, but let the top-level game coordinate route-level audio concerns:

- preserving field song time when entering battle
- restoring field song time when returning
- fading music for screen transitions
- synchronizing global song volume
- restoring default volume after special screens

Audio is part of state handoff. Treat it deliberately instead of letting every screen start and stop songs independently.

### Static Screens

Small presentation screens can remain simple:

- Title: time model, animated background, file-authored buttons.
- Credits: scroll model, song on select, quit event when finished.
- Pick save slot: file-authored layout, code updates button visibility from save-file presence.
- Intro sequence: one or more file-authored screens chained by screen transitions.

The top-level game should subscribe to their important button and lifecycle events and convert those events into route messages.
